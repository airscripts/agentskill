...
fn scan_files(files: &[agentskill_core::fs::RepoFile]) -> serde_json::Value {
    ...
    for file in files {
        ...
        tree.push(json!({"path": file.relative, "type": "file", "language": language, "language_kind": agentskill_core::language::language_kind(language), "language_role": agentskill_core::language::language_role(language), "size_bytes": file.bytes, "line_count": file.lines, "depth": Path::new(&file.relative).components().count()}));

        let entry = summary
            .entry(language.to_string())
            .or_insert_with(|| json!({"file_count": 0, "total_lines": 0}));
        entry["kind"] = json!(agentskill_core::language::language_kind(language));
        entry["role"] = json!(agentskill_core::language::language_role(language));
    ...
    for file in files {
        let language = file.language.expect("filtered files have a language").id;
        let kind = agentskill_core::language::language_kind(language)
            .expect("filtered files have a registered language")
            .as_str();
        let entry = by_kind
    ...
    json!({"tree": tree, "summary": {"total_files": files.len(), "by_language": summary, "by_kind": by_kind, "max_depth": max_depth, "avg_depth": avg_depth}, "read_order": order.into_iter().map(|item| item.1).collect::<Vec<_>>() })
}
